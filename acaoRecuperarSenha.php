...
$acao = addslashes($_GET['acao']);
	...
	if ($acao == 'recuperar_senha') {
		...
		$consultaUsuarioDB = mysql_query("SELECT * FROM usuarios WHERE email = '$email'");	
		...
		if (mysql_num_rows($consultaUsuarioDB) <= 0) {
			...
			while ($dadosUsuarioDB = mysql_fetch_array($consultaUsuarioDB)) {
				...
				$sistema = "Sistema de fornecedores";

				$email_enviar = utf8_decode("Fornecedores");
				...
				<div style='width: 100%; background: #28547f; text-align: center; padding: 7px;'>

					<h3 style='color: #fff;'>Sistema de Fornecedores - Recuperação de Senha</h3>
				...
				<div style='width: 100%; background: #fff; text-align: center; padding: 7px;'>
					...
					<p style='color: #000 !important;'>
						...
						Veja a seguir o email e a nova senha para fazer o acesso ao sistema.
					</p>
